<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">课程所属专业信息管理</div>
      ...
      <el-table :data="tableData" border :fit="true" stripe style="width: 100%"  :cell-style="{ textAlign: 'center' }" :header-cell-style="{ textAlign: 'center' }"  >
        ...
        <el-table-column fixed="right" label="操作" >
          <template slot-scope="scope">
            ...
            <el-popconfirm title="确定删除吗？" @confirm="del(scope.row.cno)" style="margin-left: 10px">
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        <el-form-item label="课程编号">
          <el-input size="small" v-model="form.cno" placeholder="请输入课程编号"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/coursesofmajor/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/coursesofmajor/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        request.post("/coursesofmajor", this.form).then(res => {
          if (res.code === '0') {
        ...
        request.post("/coursesofmajor/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(tno) {
      request.delete("/coursesofmajor/" + cno).then(res => {
        if (res.code === '0') {
