<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">选修课程信息管理</div>
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        ...
        <el-form-item label="课程号">
          <el-input size="small" v-model="form.cno" placeholder="请输入课程号"></el-input>
        </el-form-item>
        <el-form-item label="成绩">
          <el-input size="small" v-model="form.score" placeholder="成绩"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/learning/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/learning/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        request.post("/learning", this.form).then(res => {
          if (res.code === '0') {
        ...
        request.post("/learning/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(sno) {
      request.delete("/learning/" + sno).then(res => {
        if (res.code === '0') {
