<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">教师所属院系信息管理</div>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/teacherfaculties/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/teacherfaculties/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        request.post("/teacherfaculties", this.form).then(res => {
          if (res.code === '0') {
        ...
        request.post("/teacherfaculties/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(tno) {
      request.delete("/teacherfaculties/" + dno).then(res => {
        if (res.code === '0') {
