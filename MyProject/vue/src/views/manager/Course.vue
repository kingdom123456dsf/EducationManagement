<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">课程信息管理</div>
      <div style="margin-bottom: 20px; display: flex">
        <div style="flex: 10;text-align: left">
          <el-input placeholder="请输入课程编号查询" size="small"  v-model="search.cno" style="width: 20%; margin-right: 20px"><i slot="suffix" class="el-input__icon el-icon-search"></i></el-input>
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        ...
        <el-form-item label="课程名称">
          <el-input size="small" v-model="form.cname" placeholder="请输入课程名称"></el-input>
        </el-form-item>
        <el-form-item label="学分">
          <el-input size="small" v-model="form.credit" placeholder="学分"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/course/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/course/alldata").then(res => {
        if(res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(cno) {
      request.delete("/course/" + cno).then(res => {
        if (res.code === '0') {
