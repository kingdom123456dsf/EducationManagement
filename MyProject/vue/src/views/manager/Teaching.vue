<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">教授课程信息管理</div>
      ...
      <el-table :data="tableData" border :fit="true" stripe style="width: 100%"  :cell-style="{ textAlign: 'center' }" :header-cell-style="{ textAlign: 'center' }"  >
        ...
        <el-table-column prop="tclocation" label="授课地点"></el-table-column>
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        ...
        <el-form-item label="授课地点">
          <el-input size="small" v-model="form.tclocation" placeholder="请输入授课地点"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/teaching/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/teaching/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        request.post("/teaching", this.form).then(res => {
          if (res.code === '0') {
        ...
        request.post("/teaching/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(tno) {
      request.delete("/teaching/" + tno).then(res => {
        if (res.code === '0') {
