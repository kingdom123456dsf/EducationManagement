<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">专业信息管理</div>
      <div style="margin-bottom: 20px; display: flex">
        <div style="flex: 10;text-align: left">
          <el-input placeholder="请输入专业编号查询" size="small"  v-model="search.mno" style="width: 20%; margin-right: 20px"><i slot="suffix" class="el-input__icon el-icon-search"></i></el-input>
      ...
      <el-table :data="tableData" border :fit="true" stripe style="width: 100%"  :cell-style="{ textAlign: 'center' }" :header-cell-style="{ textAlign: 'center' }"  >
        <el-table-column prop="mno" label="专业编号"></el-table-column>
        ...
        <el-table-column fixed="right" label="操作" >
          <template slot-scope="scope">
            ...
            <el-popconfirm title="确定删除吗？" @confirm="del(scope.row.mno)" style="margin-left: 10px">
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        <el-form-item label="专业编号">
          <el-input size="small" v-model="form.mno" placeholder="请输入专业编号"></el-input>
        </el-form-item>
        <el-form-item label="专业名称">
          <el-input size="small" v-model="form.mname" placeholder="请输入专业名称"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/major/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/major/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        ...
        request.post("/major/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(mno) {
      request.delete("/major/" + mno).then(res => {
        if (res.code === '0') {
