<template>
  <div>
    <div style="width: 100%;height:1000px;padding:40px 40px; background-color: white;border-radius: 4px">
      <!--按钮 和 搜索框样式-->
      <div style="margin-bottom: 40px;font-size: 18px;font-weight: 530;">学生信息管理</div>
      <div style="margin-bottom: 20px; display: flex">
        <div style="flex: 10;text-align: left" >
          <el-input placeholder="请输入学号查询" size="small"  v-model="search.sno" style="width: 20%; margin-right: 20px"><i slot="suffix" class="el-input__icon el-icon-search"></i></el-input>
      ...
      <el-table :data="tableData" border :fit="true" stripe style="width: 100%"  :cell-style="{ textAlign: 'center' }" :header-cell-style="{ textAlign: 'center' }"  >
        ...
        <el-table-column prop="slocation" label="家庭地址"></el-table-column>
...
<!--            <el-button @click="handleClick(scope.row)" type="text" size="small">查看</el-button>-->
            ...
            <el-popconfirm title="确定删除吗？" @confirm="del(scope.row.sno)" style="margin-left: 10px">
    ...
    <el-dialog title="请填写信息" :visible.sync="dialogVisible" width="40%">
      <el-form :model="form" label-position="right" label-width="100px" style="padding-right: 40px">
        <el-form-item label="学号">
          <el-input size="small" v-model="form.sno" placeholder="请输入学号"></el-input>
        </el-form-item>
        <el-form-item label="姓名">
          <el-input size="small" v-model="form.sname" placeholder="请输入姓名"></el-input>
        </el-form-item>
        <el-form-item label="性别">
          <el-radio-group size="small" v-model="form.ssex">
            <el-radio label="男"></el-radio>
            <el-radio label="女"></el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="年龄">
          <el-input size="small" v-model="form.sage" placeholder="请输入年龄"></el-input>
        </el-form-item>
        <el-form-item label="家庭地址">
          <el-input size="small" v-model="form.slocation" placeholder="请输入家庭地址"></el-input>
        </el-form-item>
        <el-form-item label="民族">
          <el-input size="small" v-model="form.snation" placeholder="请输入民族"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    // 分页加载表格数据
    load() {
      request.post("/student/page?pageNum=" + this.pageNum, this.search).then(res => {
        if (res.code === '0') {
    ...
    /*load() {
      request.get("/student/alldata").then(res => {
        if(res.code === '0') {
    ...
    save() {
      if (!this.form.isEdit) {
        ...
        request.post("/student/edit", this.form).then(res => {
          if (res.code === '0') {
    ...
    // 根据ID删除某条数据
    del(sno) {
      request.delete("/student/" + sno).then(res => {
        if (res.code === '0') {
